...
class DfuCommandHandler
{
    ...
    {
        $btAddress = $args->getArgument('address');
        $dfuFile = $args->getArgument('file');

        $runner = new Runner(null, $io, $this->flysystem);
        $runner->generateDfuXML($btAddress, $dfuFile);

        return $runner->runDfuSuite($dfuFile);
    }
}